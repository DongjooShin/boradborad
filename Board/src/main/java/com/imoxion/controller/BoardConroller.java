...
public class BoardConroller {
	...
	public String listAll(Model model, @RequestParam("cpage") int cpage) {
		...
		Criteria criteria = boardService.getPaging(rowPerPage,cpage);
		try {
			list = boardService.getListService(criteria);
		} catch (Exception e) {
			e.printStackTrace();
		}
...
*/
 ...
 System.out.println(board.toString());
	...
	@RequestMapping(value = "/SearchListAll", method = RequestMethod.GET)
	public String SearchListAll(Model model,@RequestParam("cpage") int cpage,
			@RequestParam("searchType") String searchType,@RequestParam("keyword") String keyword) {
		
		int rowPerPage =3;
		List<BoardVO> list = null;
		SearchCriteria searchCriteria =  boardService.getSearch(searchType,keyword,rowPerPage,cpage);
		list = boardService.getSearchListService(searchCriteria);
